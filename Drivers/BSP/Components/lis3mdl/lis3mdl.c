...
{
	//debug laused juurde lisatud.
    printf("Initializing LIS3MDL...\r\n");

    /* Operating mode selection - power down */
    if (lis3mdl_operating_mode_set(&(pObj->Ctx), LIS3MDL_POWER_DOWN) != LIS3MDL_OK)
    {
        printf("Failed to set operating mode to power down\r\n"); //jõudis korra ka siia aga ilmselt vale aadressiga
    ...
    {
        printf("Failed to enable block data update\r\n");
        return LIS3MDL_ERROR;
    }
